...
public class ListUserAction extends AbstractAuthenticatedAction{
    ...
    public void perform(){
        if(state.getUserRole()== AbstractAuthenticatedAction.Role.ADMINISTRATOR){
            User user=null;
            ...
            if(user!=null) {
                AbstractAuthenticatedAction.Role role = user.getRole();
                ...
                System.out.printf("%-10s %-10s %-15s %-16s","role","email","telephoneNumber","registrationTime");
                System.out.println();
                System.out.printf("%-10s %-10s %-15s %-16s",role,email,telephoneNumber,registrationTime);
                System.out.println();
            }
